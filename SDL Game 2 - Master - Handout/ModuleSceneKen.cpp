...
ModuleSceneKen::ModuleSceneKen(bool start_enabled) : Module(start_enabled)
{
	...
	girl.frames.push_back({ 624, 79, 32, 56 });
	girl.frames.push_back({ 624, 143, 32, 56 });
	girl.speed = 0.08f;
}
...
update_status ModuleSceneKen::Update()
{
	...
	if (shippixmov == 0 || shippixmov == 500)
		shipmov = !shipmov;
	if (shipmov)
		shippixmov++;
	else
	...
	App->renderer->Blit(graphics, 0, shippixmov/100, &ship, 0.85f); // ship

	// TODO 6: Draw the girl. Make sure it follows the ship movement!
	App->renderer->Blit(graphics, 192, 104 + shippixmov / 100, &(girl.GetCurrentFrame()), 0.85f); // girl animation
